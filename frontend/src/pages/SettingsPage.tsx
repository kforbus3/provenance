...
function BrandingCard({ current }: { current: unknown }) {
  ...
  const [saved, setSaved] = useState(false);
  const save = useMutation({
    mutationFn: () => setSetting("branding", { app_name: name.trim() || "Provenance" }),
    onSuccess: () => {
      setSaved(true);
    ...
    <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
      ...
      <Stack direction="row" spacing={2} alignItems="flex-start">
        <TextField
          ...
          placeholder="Provenance" sx={{ flexGrow: 1, maxWidth: 360 }}
        />
...
function ITSMCard() {
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5, mb: 1.5 }}>
        When enabled, Provenance opens a change/incident ticket for each just-in-time access request and
    ...
    return (
      <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
        ...
        <Alert severity="error" action={<Button size="small" onClick={() => void refetch()}>Retry</Button>}>
          Couldn't load the current configuration. If you just updated Provenance, hard-refresh the page
...
function SCIMCard() {
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5, mb: 1.5 }}>
        Let your identity provider create, update, and — importantly — deprovision Provenance accounts
