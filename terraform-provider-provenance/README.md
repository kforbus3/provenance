# Terraform Provider for Provenance

Manage a [Provenance](https://github.com/kforbus3/Provenance-Terminal) deployment as
...
from **Settings → Service Accounts** in the Provenance UI. Grant the token a role with
only the permissions your configuration needs (least privilege):

- `prov_host` → `Host.Enroll` / `Host.Edit` / `Host.Delete` / `Host.View`
- `prov_group` → `Group.Create` / `Group.Edit` / `Group.Delete`
- `prov_service_account` / `..._token` → `ServiceAccount.Manage`
- `data.prov_role` → `Role.Edit`
...
| Type | Kind | Notes |
|---|---|---|
| `prov_host` | resource | Full CRUD + `terraform import <id>`. |
| `prov_group` | resource | Full CRUD; add a `rule { }` block for dynamic membership. |
| `prov_service_account` | resource | Create/Delete; **any change replaces it** (the API has no in-place update). |
| `prov_service_account_token` | resource | Create/Revoke; the secret is stored (sensitive) in state and shown once. |
| `prov_role` | data source | Resolve a role name to its UUID (for `role_ids`). |

See [`examples/main.tf`](./examples/main.tf) for a complete configuration.

## Building from source

This provider lives in the Provenance monorepo and builds against the local SDK
via a `replace github.com/kforbus3/Provenance-Terminal/sdk => ../sdk` directive, so build
it from a checkout of the monorepo:

```bash
cd terraform-provider-provenance
go build -o terraform-provider-provenance
```
...
separate from the offline `.provup` bundle key.)
...
provider_installation {
  dev_overrides {
    "kforbus3/provenance" = "/absolute/path/to/terraform-provider-provenance"
  }
  direct {}
}
```
