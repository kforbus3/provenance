...
// clientFromProviderData extracts the configured *prov.Client passed from the
// provider's Configure. It is nil during validation (providerData unset), which
// callers must tolerate.
func clientFromProviderData(providerData any, diags *diag.Diagnostics) *prov.Client {
	if providerData == nil {
		return nil
	}
	client, ok := providerData.(*prov.Client)
	if !ok {
		diags.AddError("Unexpected provider data",
			fmt.Sprintf("Expected *prov.Client, got %T. This is a provider bug.", providerData))
		return nil
	}
	return client
}
