...
// stand up real resources against a live Provenance deployment. They are gated on
...
//	            {ResourceName: "prov_host.web", ImportState: true, ImportStateVerify: true},
//	        },
//	    })
//	}
//
...
// require TF_ACC plus valid Provenance credentials.
func testAccPreCheck(t *testing.T) {
	t.Helper()
	if os.Getenv("TF_ACC") == "" {
		t.Skip("acceptance tests skipped; set TF_ACC=1 to run them")
	}
	for _, v := range []string{"PROV_URL", "PROV_API_TOKEN"} {
		if os.Getenv(v) == "" {
