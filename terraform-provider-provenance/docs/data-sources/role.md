---
page_title: "prov_role Data Source - fleet"
description: |-
  Resolve a Provenance role name to its UUID.
---

# prov_role (Data Source)

Looks up a role by name and returns its UUID, for use in `role_ids` on service
accounts. The Provenance API returns role *names* rather than IDs on read, so this
