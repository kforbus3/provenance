#!/bin/sh
# prov-wg-persist.sh — make a managed host's Provenance WireGuard tunnel survive
...
#     sudo sh prov-wg-persist.sh            # defaults to interface "wgprov"
#     sudo sh prov-wg-persist.sh wg0        # or name the interface
#
# It installs:
...
#   - /usr/local/sbin/prov-wg-reresolve + a 30s systemd timer that brings the
...
cat > "/etc/systemd/system/wg-quick@${IF}.service.d/prov.conf" <<'DROPIN'
[Service]
Environment=WG_QUICK_USERSPACE_IMPLEMENTATION=wireguard-go
DROPIN

cat > /usr/local/sbin/prov-wg-reresolve <<'SCRIPT'
#!/bin/sh
IF="${1:-wgprov}"
CONF="/etc/wireguard/${IF}.conf"
[ -f "$CONF" ] || exit 0
...
[Service]
Type=oneshot
ExecStart=/usr/local/sbin/prov-wg-reresolve ${IF}
UNIT
...
systemctl enable --now prov-wg-reresolve.timer >/dev/null 2>&1 || true
# Bring it current now, too.
/usr/local/sbin/prov-wg-reresolve "$IF" || true

echo "Installed. wg-quick@${IF} enabled on boot; prov-wg-reresolve.timer active."
echo "Verify: systemctl is-enabled wg-quick@${IF} ; systemctl status prov-wg-reresolve.timer"
