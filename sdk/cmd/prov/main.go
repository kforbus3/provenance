// Command fleet is the remote automation CLI for Provenance. It authenticates
...
//
// This is distinct from `provctl`, the on-host/break-glass tool that talks to the
...
//
//	PROV_URL         base URL of the deployment (e.g. https://provenance.example.com)
//	PROV_API_TOKEN   service-account API token (flt_...)
//
// Examples:
//
//	fleet hosts list
...
func main() {
	...
	if err := run(ctx, os.Args[1], os.Args[2:]); err != nil {
		...
		if prov.IsUnauthorized(err) {
			fmt.Fprintln(os.Stderr, "hint: set PROV_API_TOKEN to a valid service-account token with the required permission.")
		}
		os.Exit(1)
	}
}
...
func client() (*prov.Client, error) {
	base := os.Getenv("PROV_URL")
	if base == "" {
		return nil, errors.New("PROV_URL is not set (e.g. https://provenance.example.com)")
	}
	return prov.New(base,
		prov.WithToken(os.Getenv("PROV_API_TOKEN")),
		prov.WithUserAgent("fleet-cli/"+version),
	)
}

func usage() {
	fmt.Fprint(os.Stderr, `fleet — remote automation CLI for Provenance
