...
func TestSupersededImages(t *testing.T) {
	const prefix = "fleet-terminal-"
	comps := []string{"backend", "frontend", "grype-scanner", "ansible-runner", "fleet-updater"}

	tags := []string{
		// Superseded versions of our own components.
		"fleet-terminal-backend:1.0.0",
		"fleet-terminal-backend:1.1.0",
		"fleet-terminal-frontend:1.1.0",
		"fleet-terminal-ansible-runner:0.70.2",
		// The version just installed: running.
		"fleet-terminal-backend:1.2.1",
		"fleet-terminal-frontend:1.2.1",
		// The rollback anchor: the only route back if the new version is bad.
		"fleet-terminal-backend:rollback",
		"fleet-terminal-frontend:rollback",
...
func TestSupersededImagesKeepsWhatMatters(t *testing.T) {
	const prefix = "fleet-terminal-"
	comps := []string{"backend", "fleet-updater"}

	mustKeep := map[string]string{
		"fleet-terminal-backend:1.2.1":      "the version just installed is RUNNING",
		"fleet-terminal-backend:rollback":   "the rollback anchor is the only way back from a bad upgrade",
		...
		"fleet-terminal-backend:latest":     "a floating tag is not a superseded version",
		"fleet-terminal-backendish:1.0.0":   "a component whose name merely shares a prefix",
		"blackfriars-builder-runner:latest": "the image builder sidecar",
	}
	...
	got := supersededImages([]string{"fleet-terminal-backend:1.1.0"}, prefix, "1.2.1", comps)
	if len(got) != 1 {
...
func TestSupersededImagesOnlyCoversBundledComponents(t *testing.T) {
	got := supersededImages(
		[]string{"fleet-terminal-frontend:1.1.0", "fleet-terminal-backend:1.1.0"},
		"fleet-terminal-", "1.2.1", []string{"backend"})
	if len(got) != 1 || got[0] != "fleet-terminal-backend:1.1.0" {
