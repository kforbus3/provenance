// Command prov-enroll-agent enrolls a managed host into Provenance using
...
func run() error {
	var (
		base      = flag.String("url", "", "backend base URL, e.g. https://provenance.example.com")
		host      = flag.String("host", "", "host id (UUID) or hostname to enroll")
		token     = flag.String("token", os.Getenv("PROV_TOKEN"), "access token (or set PROV_TOKEN)")
	...
	if *insecure && fipsEnabled() {
		return fmt.Errorf("-insecure is not allowed in FIPS mode (PROV_FIPS_MODE): peer certificates must be verified")
	}
...
// fipsEnabled reports whether PROV_FIPS_MODE is set to a truthy value, mirroring
// the backend's config parsing (strconv.ParseBool) so the agent enforces the same
// TLS posture the backend runs under.
func fipsEnabled() bool {
	if v, ok := os.LookupEnv("PROV_FIPS_MODE"); ok {
