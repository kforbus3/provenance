...
Usage:
  ...
  fleetctl support-bundle [--out FILE] [--anonymise]      Write a support bundle about this instance (--anonymise masks hostnames + IPs)
...
func run(cmd string, args []string) error {
	...
	case "support-bundle":
		...
		return supportBundleCmd(ctx, pool, st, cfg, os.Getenv("FLEET_VERSION"), out, anonymise)
	case "create-admin":
		if len(args) < 2 {
