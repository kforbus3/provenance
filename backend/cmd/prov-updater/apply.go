...
func (u *Updater) Apply(ctx context.Context, req ApplyReq) {
	...
	var sidecars []string
	for _, c := range m.Components {
		// prov-updater is handled last, via a detached helper — it cannot recreate its
...
const updaterComponent = "prov-updater"

// selfUpdate replaces the prov-updater container with newImage, out of band. The
// updater can't run `compose up prov-updater` itself — that recreate would kill this
...
func (u *Updater) selfUpdate(ctx context.Context, newImage string) error {
	...
	// --project-directory is the REAL host compose dir so the prov-updater service's
