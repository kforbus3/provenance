// Command prov-updater is the privileged sidecar that applies signed Provenance
...
func loadConfig() Config {
	return Config{
		Listen:          env("PROV_UPDATER_LISTEN", ":9000"),
		Token:           os.Getenv("PROV_UPDATER_TOKEN"),
		Env:             env("PROV_ENV", "development"),
		UpdatesDir:      env("PROV_UPDATES_DIR", "/var/lib/prov/updates"),
		Project:         env("PROV_UPDATER_PROJECT", "provenance"),
		ComposeFiles:    splitList(env("PROV_UPDATER_COMPOSE_FILES", "/compose/docker-compose.yml:/compose/docker-compose.jumphost.yml")),
		EnvFile:         env("PROV_UPDATER_ENV_FILE", "/compose/.env"),
		BackendURL:      env("PROV_UPDATER_BACKEND_URL", "http://backend:8080"),
		BackendServices: splitList(env("PROV_UPDATER_BACKENDS", "backend")),
...
func main() {
	...
	cfg := loadConfig()
	trusted, err := release.TrustedKeys(os.Getenv("PROV_RELEASE_TRUST_KEYS"))
	if err != nil {
		log.Error("prov-updater: bad PROV_RELEASE_TRUST_KEYS", "err", err)
		os.Exit(1)
	}
	if len(trusted) == 0 {
		log.Warn("prov-updater: no trusted release keys configured — every upgrade will be rejected until a key is set")
	}
	if cfg.Token == "" {
		...
		if cfg.Env != "development" {
			log.Error("prov-updater: PROV_UPDATER_TOKEN is required outside development — refusing to start (set it so only the backend can trigger upgrades)", "env", cfg.Env)
			os.Exit(1)
		}
		log.Warn("prov-updater: PROV_UPDATER_TOKEN is empty — set it so only the backend can trigger upgrades")
	}
	...
	log.Info("prov-updater listening", "addr", cfg.Listen, "project", cfg.Project)
	srv := &http.Server{Addr: cfg.Listen, Handler: mux, ReadHeaderTimeout: 10 * time.Second}
	if err := srv.ListenAndServe(); err != nil {
		log.Error("prov-updater server exited", "err", err)
		os.Exit(1)
	}
}
