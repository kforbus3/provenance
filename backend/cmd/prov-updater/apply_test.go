...
func makeBundleCompat(t *testing.T, compat string) (path string, pub ed25519.PublicKey) {
	...
	mk := func(name, content string) release.ImageRef {
		...
		return release.ImageRef{Component: name, Image: "provenance-" + name, Tag: "v1.2.3", File: "images/" + name + ".tar", Digest: dg, Bytes: sz}
	}
	...
	path = filepath.Join(dir, "b.provup")
	f, _ := os.Create(path)
...
func newUpdater(t *testing.T, pub ed25519.PublicKey, d Docker, h Health) *Updater {
	return &Updater{
		cfg:    Config{UpdatesDir: t.TempDir(), Project: "provenance", HealthTimeout: time.Second},
...
func makeBundleWith(t *testing.T, components []string, adds []release.ConfigAddition) (string, ed25519.PublicKey) {
	...
	for _, c := range components {
		...
		dg, sz, _ := release.HashFile(p)
		imgs = append(imgs, release.ImageRef{Component: c, Image: "provenance-" + c, Tag: "v1.2.3", File: "images/" + c + ".tar", Digest: dg, Bytes: sz})
...
func TestApplySelfUpdate(t *testing.T) {
	// A bundle that includes the prov-updater component must NOT recreate the updater
	// inline (that would kill the apply mid-flight) — it hands off to a detached helper.
	path, pub := makeBundleWith(t, []string{"backend", "prov-updater"}, nil)
	fd := &fakeDocker{}
	u := newUpdater(t, pub, fd, &fakeHealth{})
	u.Apply(context.Background(), ApplyReq{Bundle: path, CurrentVersion: "v1.1.0", TargetVersion: "v1.2.3"})
	...
	for _, svcs := range fd.composedUp {
		for _, s := range svcs {
			if s == "prov-updater" {
				t.Fatalf("prov-updater must not be recreated inline: %v", fd.composedUp)
			}
		}
	}
	if len(fd.detachedImgs) != 1 || fd.detachedImgs[0] != "provenance-prov-updater:v1.2.3" {
		t.Fatalf("self-update handoff image wrong: %v", fd.detachedImgs)
	}
	if len(fd.detachedCmds) != 1 || !strings.Contains(fd.detachedCmds[0], "up -d --no-build --no-deps prov-updater") {
...
func TestMergeConfigAdditions(t *testing.T) {
	...
	adds := []release.ConfigAddition{
		{Key: "PROV_A", Default: "overwrite-me"}, // exists -> skipped
		{Key: "PROV_C", Default: "new", Comment: "the c setting"},
		{Key: "PROV_TOK", Generate: "secret"},
	}
	out, added, err := mergeConfigAdditions(env, adds)
	if err != nil {
		t.Fatal(err)
	}
	if len(added) != 2 || added[0] != "PROV_C" || added[1] != "PROV_TOK" {
		t.Fatalf("added wrong: %v", added)
	}
	if !strings.Contains(out, "PROV_A=1") || strings.Contains(out, "overwrite-me") {
		t.Errorf("existing key changed:\n%s", out)
	}
	if !strings.Contains(out, "# the c setting\nPROV_C=new") {
		t.Errorf("comment+key not written:\n%s", out)
	}
	idx := strings.Index(out, "PROV_TOK=")
	if idx < 0 {
		t.Fatalf("secret not written:\n%s", out)
	}
	val := strings.SplitN(strings.TrimSpace(out[idx+len("PROV_TOK="):]), "\n", 2)[0]
	if len(val) != 64 {
...
func TestWriteOverride(t *testing.T) {
	...
	if err := writeOverride(p, map[string]string{"backend": "provenance-backend:v1.2.3"}); err != nil {
		t.Fatal(err)
	}
	b, _ := os.ReadFile(p)
	got := string(b)
	for _, want := range []string{"backend:", "image: provenance-backend:v1.2.3", "pull_policy: never"} {
