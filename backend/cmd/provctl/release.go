...
// runRelease implements `provctl release <keygen|build>` — the offline publisher
// tooling that produces the signed .provup upgrade bundles the in-UI updater applies.
func runRelease(args []string) error {
	if len(args) < 1 {
		return fmt.Errorf("usage: provctl release <keygen|build> [flags]")
	}
	switch args[0] {
	case "keygen":
...
// releaseChannel builds and signs a release-channel index from one or more .provup
// bundles, for hosting at PROV_UPDATE_CHANNEL_URL so instances can "check for
...
func releaseChannel(args []string) error {
	...
	if *keyPath == "" || *baseURL == "" || len(bundles) == 0 {
		...
		return fmt.Errorf("usage: provctl release channel --key <priv> --base-url <url> [--out channel.json] <bundle.provup>...")
	}
...
// key (or PROV_RELEASE_TRUST_KEYS) and prints its manifest. Read-only; safe for CI.
func releaseVerify(args []string) error {
	fs := flag.NewFlagSet("release verify", flag.ContinueOnError)
	bundle := fs.String("bundle", "", "path to the .provup bundle (required)")
	pubKeys := fs.String("keys", os.Getenv("PROV_RELEASE_TRUST_KEYS"), "base64 trusted public key(s), comma-separated (default $PROV_RELEASE_TRUST_KEYS)")
	...
	if *extract {
		dir, err := os.MkdirTemp("", "provup-verify-*")
		if err != nil {
			return err
		}
...
// configuring via PROV_RELEASE_TRUST_KEYS.
func releaseKeygen(args []string) error {
	...
	fmt.Printf("public key (bake into builds / set PROV_RELEASE_TRUST_KEYS):\n%s\n", release.EncodePublicKey(pub))
	return nil
}

// releaseBuild assembles and signs a .provup bundle from already-built, version-tagged
...
func releaseBuild(args []string) error {
	...
	out := fs.String("out", "", "output bundle path, e.g. provenance-v0.61.0.provup (required)")
	components := fs.String("components", "backend,frontend,grype-scanner", "comma-separated components to include")
	prefix := fs.String("image-prefix", "provenance", "image name prefix (images are <prefix>-<component>:<version>)")
	...
	staging, err := os.MkdirTemp("", "provup-*")
	if err != nil {
		return err
	}
