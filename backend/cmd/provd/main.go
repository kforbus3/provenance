// Command provd is the Provenance API server and SSH gateway.
package main
...
func run() error {
	...
	log.Info("starting provd", "version", version, "env", cfg.Environment)
	...
	// leaked. No effect when PROV_MULTI_TENANCY is off.
	ctx = tenant.WithBypass(ctx)

	// Unwrap any KMS-protected master passphrases before the CA or credential vault is
	// used. No-op unless an external KMS backend is configured (PROV_KMS_PROVIDER).
	if cfg.KMSEnabled() {
	...
	shutdownTracing, err := telemetry.SetupTracing(ctx, cfg.TracingOn, cfg.OTLPEndpoint, "provenance-backend", version, log)
	if err != nil {
		return err
	}
	defer func() {
