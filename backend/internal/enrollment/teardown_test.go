...
func TestTeardownDetachesFromTheSSHSession(t *testing.T) {
	...
	if !strings.Contains(s, "setsid nohup /usr/local/sbin/prov-unenroll.sh") {
...
// standing NOPASSWD root account and a trusted CA on a machine Provenance no longer
...
func TestTeardownRemovesEverythingEnrollmentInstalled(t *testing.T) {
	...
	if !strings.Contains(s, "/etc/ssh/prov_krl") {
...
// removes. It must touch only what Provenance wrote, and must not reload sshd into a
...
func TestTeardownDoesNotStrandTheHost(t *testing.T) {
	...
	for _, forbidden := range []string{"authorized_keys", "/root", "PermitRootLogin", "/etc/passwd", "/etc/shadow"} {
		if strings.Contains(s, forbidden) {
			t.Errorf("teardown touches %q, which Provenance did not install", forbidden)
		}
	}
	// The sudoers removal must be the single file Provenance wrote, never the directory
	// or the main file.
	if strings.Contains(s, "rm -f /etc/sudoers ") || strings.Contains(s, "rm -rf /etc/sudoers.d") {
		t.Error("teardown must remove only /etc/sudoers.d/prov")
	}
	...
	if !strings.Contains(s, "mv -f /etc/ssh/sshd_config.prov-backup /etc/ssh/sshd_config") {
...
// The awk that strips Provenance's appended sshd_config block must remove exactly that
// block: the marker and the three directives Provenance wrote under it, stopping at the
...
func TestTeardownStripsOnlyFleetsSSHDBlock(t *testing.T) {
	...
	end := strings.Index(script[start:], "' /etc/ssh/sshd_config.prov-backup")
	if start < 0 || end < 0 {
...
`
	...
	if strings.Contains(got, "# Provenance") || strings.Contains(got, "/etc/ssh/prov_ca.pub") {
		t.Errorf("Provenance's block survived:\n%s", got)
	}
	// The operator's own settings — including a PubkeyAuthentication identical to
	// the one in Provenance's block, and a CA line placed after it — must all remain.
	...
	// A config with no Provenance block must come back byte-identical.
	...
	if string(out) != untouched {
		t.Errorf("a config with no Provenance block was modified:\ngot  %q\nwant %q", out, untouched)
	}
}
...
// deleted from Provenance and kept a live, handshaking tunnel onto the fleet's network,
// with nothing left on it that Provenance managed or audited — while the documentation
// and the manual script both said the transport was retired.
func TestTeardownRetiresTheOverlay(t *testing.T) {
	wgSvc := &Service{cfg: &config.Config{WGInterface: "wgprov"}}
	...
	for _, name := range []string{"", "wireguard"} {
		...
		if !strings.Contains(script, "IF=wgprov") {
	...
	certSvc := &Service{
		cfg:      &config.Config{WGInterface: "wgprov"},
...
// The in-app teardown and scripts/prov-unenroll.sh remove the same things by two
...
func TestTeardownPathsDoNotDrift(t *testing.T) {
	manualPath := filepath.Join("..", "..", "..", "scripts", "prov-unenroll.sh")
	...
	for _, artifact := range []struct{ what, token string }{
		{"the sudoers grant", "/etc/sudoers.d/prov"},
		{"the trusted CA", "/etc/ssh/prov_ca.pub"},
		{"the revocation list", "/etc/ssh/prov_krl"},
		{"the principal files", "/etc/ssh/auth_principals"},
		{"the sshd drop-in", "/etc/ssh/sshd_config.d/00-prov.conf"},
		{"the appended sshd block", "# (Provenance|Fleet Terminal)$"},
		// Both generations. A host enrolled before the rename carries the fleet_*
		// names; a teardown that removes only the current spelling leaves a trusted
		// CA and a NOPASSWD sudoers entry on a host the operator believes is clean.
		{"the pre-rename sudoers grant", "/etc/sudoers.d/fleet"},
		{"the pre-rename trusted CA", "/etc/ssh/fleet_ca.pub"},
		{"the pre-rename revocation list", "/etc/ssh/fleet_krl"},
		{"the pre-rename sshd drop-in", "/etc/ssh/sshd_config.d/00-fleet.conf"},
		...
		{"the WireGuard config", ".prov-disabled"},
		...
		if !strings.Contains(manual, artifact.token) {
			t.Errorf("scripts/prov-unenroll.sh does not handle %s (%q)", artifact.what, artifact.token)
		}
	}
}
...
// (which keeps ca/cert/key on purpose), and scripts/prov-unenroll.sh did not mention
...
func TestOpenVPNTeardownPathsDoNotDrift(t *testing.T) {
	manualBytes, err := os.ReadFile(filepath.Join("..", "..", "..", "scripts", "prov-unenroll.sh"))
	if err != nil {
	...
	for _, artifact := range []struct{ what, token string }{
		...
		{"the renamed client config", "client.ovpn.prov-disabled"},
		{"the peer-isolation hook", "peer-isolation.sh"},
		{"the systemd client unit", "openvpn-client@prov-overlay"},
		{"the peer-isolation chains", "PROV-OVPN-IN"},
	} {
		if !strings.Contains(inApp, artifact.token) {
