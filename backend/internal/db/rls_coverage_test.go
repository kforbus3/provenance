...
var rlsGlobalAllowlist = map[string]string{
	// --- container update rollouts (0090, 0092) ---
	"container_update_rollout_images": "the images one container-update rollout covers; reachable only via container_update_rollouts, which is RLS-scoped. A tenant_id here would be a second copy of the parent's and a second place for the two to disagree (0092).",
	...
	"registry_tag_cache":      "a public registry's tag listing, keyed by repository and holding no tenant data: the registry publishes the same tags to everyone. Shared deliberately, and for a sharper reason than the others — following pagination made a listing cost up to 32 requests per repository, so a per-tenant copy would multiply the exact rate-limit pressure this table exists to relieve. It records repository NAMES, which a tenant could in principle read as a hint that some other tenant runs a given image; that is not reachable through any API, and the alternative is every tenant re-listing 30,000 tags to learn what is already public (0096).",
