...
-- carry (functions prov_current_tenant/prov_rls_visible are defined in 0051). With
-- PROV_MULTI_TENANCY off the app sets app.tenant_id='bypass' and every row is
-- visible; with it on, a tenant sees only its own hosts' availability history.
DO $$
BEGIN
  IF to_regclass('host_status_events') IS NOT NULL
     AND EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'prov_current_tenant')
     AND EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'prov_rls_visible') THEN
    ...
    ALTER TABLE host_status_events ALTER COLUMN tenant_id SET DEFAULT prov_current_tenant();
