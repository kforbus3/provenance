...
// Provenance itself restarted mid-run — e.g. a playbook that reboots the hypervisor
// hosting Provenance), so the run's result was never collected and the target hosts
...
func (s *Store) FailStalePlaybookRuns(ctx context.Context, lease time.Duration, self uuid.UUID) (int64, error) {
	tag, err := s.pool.Exec(ctx,
		`UPDATE playbook_runs SET status='interrupted', error='Provenance restarted mid-run — the run was cut off and its result was not collected; the target hosts may still have completed their tasks', finished_at=now()
