...
SH 70b-openvpn.txt 'echo "== client status =="; systemctl status openvpn@prov-overlay openvpn-client@prov-overlay 2>/dev/null | head -30; echo; echo "== tunnel devices =="; ip -4 -br addr show 2>/dev/null | grep -E "^(tun|tap)" ; echo; echo "== recent client log =="; tail -n 40 /etc/openvpn/prov/client.log 2>/dev/null'
