...
func (h *handler) link(w http.ResponseWriter, r *http.Request) {
	...
	opts := host.Options
	opts.FlipsideMachineID = id
	if _, err := h.svc.store.UpdateHost(r.Context(), host.ID, store.HostInput{
		Hostname: host.Hostname, Description: host.Description, Environment: host.Environment,
		Owner: host.Owner, Address: host.Address, SSHPort: host.SSHPort, SSHUser: host.SSHUser,
		Tags: host.Tags, Protocol: host.Protocol, RDPPort: host.RDPPort,
		RDPOptions: host.RDPOptions, Options: opts,
		AuthMethod: host.AuthMethod, CredentialID: host.CredentialID,
	}); err != nil {
