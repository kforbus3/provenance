...
Federation turns one Blackfriars instance into a **hub** — a single pane of
glass over many independent **site** instances, each a full Blackfriars stack on its
...
Each stack — hub and every site — is a full Blackfriars deployment with its own database and
