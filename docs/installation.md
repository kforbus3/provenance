...
This guide takes you from nothing to a running Blackfriars you can sign in to.
...
- **A local Ollama instance** if you want the "Ask Blackfriars" AI assistant (configured in the UI, not required to run).

> Blackfriars is the SSH control plane for your fleet. Install it on a host you
...
— Blackfriars shows a one-time **bootstrap** page. Create your first account there;
...
2. On the Blackfriars **Vulnerabilities** page, use **Import offline** to upload that archive.
...
You now have a running Blackfriars. To make it useful:
