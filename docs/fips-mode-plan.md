...
> Note: a strongSwan/IPsec provisioner was prototyped but **removed** because it could
> not be validated end-to-end without a real IPsec (kernel-`xfrm`) capable host. The
> overlay abstraction remains, so a second cert overlay can be added later once it can
> be properly tested.
- **`internal/overlaypki`** — an X.509 CA (ECDSA P-256) distinct from the SSH CA (OpenVPN
...
- **`internal/overlay`** — an `Overlay` interface (`EnsureServer` / `ProvisionHost`)
  ...
  `tls-groups secp256r1:secp384r1` — OpenSSL 3 ignores the deprecated `ecdh-curve` and
...
- **Server boot** builds the provisioner sharing the PKI; the overlay CA is created
  **lazily** (only when a host actually uses the cert overlay), pre-warmed on boot only when
...
- Validated end-to-end in Docker: OpenVPN PKI chain + live tunnel (mutual ECDSA auth,
  ...
  Blackfriars's actual provisioning scripts on real containers; overlay-CA persistence/reload
...
- **Host-OS FIPS OpenSSL images** — the jump host and managed hosts must run a
  FIPS-validated OpenSSL for the whole trust chain to be FIPS; Blackfriars documents this
...
Add an opt-in **FIPS mode** in which *all* cryptography Blackfriars performs uses FIPS 140-3
...
FIPS — the Blackfriars backend image (validated module), the jump host, and every managed
...
with certs from Blackfriars's ECDSA CA and use approved suites (AES-256-GCM, ECDH/ECDHE
...
— clients present a cert issued by Blackfriars's (now ECDSA) CA. This is a nice synergy: the
overlay and SSH share the same FIPS CA, and the per-host identity Blackfriars already persists
...
- The jump-host image runs an **OpenVPN server** (FIPS OpenSSL) instead of / alongside
  the WG hub. It trusts the Blackfriars CA and issues/accepts client certs.
...
overlay selected — any method, including no-install. Blackfriars then:
...
- renumbers the host into the joining overlay's pool (its address cannot follow it
  ...
  overlay addresses are recycled;
- proves the new tunnel by dialing the host's new overlay address **from the jump
