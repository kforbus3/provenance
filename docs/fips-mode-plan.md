...
- Validated end-to-end in Docker: OpenVPN PKI chain + live tunnel (mutual ECDSA auth,
  ...
  Provenance's actual provisioning scripts on real containers; overlay-CA persistence/reload
...
- **Host-OS FIPS OpenSSL images** — the jump host and managed hosts must run a
  FIPS-validated OpenSSL for the whole trust chain to be FIPS; Provenance documents this
...
FIPS — the Provenance backend image (validated module), the jump host, and every managed
...
— clients present a cert issued by Provenance's (now ECDSA) CA. This is a nice synergy: the
overlay and SSH share the same FIPS CA, and the per-host identity Provenance already persists
...
- The jump-host image runs an **OpenVPN server** (FIPS OpenSSL) instead of / alongside
  the WG hub. It trusts the Provenance CA and issues/accepts client certs.
...
overlay selected — any method, including no-install. Provenance then:
