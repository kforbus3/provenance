# Exposing Provenance to the Internet

This guide covers putting the Provenance web UI on the public internet behind
...
- **Renewal is on you.** Unlike Let's Encrypt (which NPM auto-renews), a custom cert
  ...
  it against the NPM API). Provenance's Expiry & Rotation dashboard tracks *Provenance's* CA
...
After bootstrap the host trusts the Provenance CA and every user connects with their
