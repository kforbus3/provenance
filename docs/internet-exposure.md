# Exposing Blackfriars to the Internet

This guide covers putting the Blackfriars web UI on the public internet behind
...
- **The certificate's SAN (or CN) must match the hostname in `FLEET_PUBLIC_URL`.**
  Blackfriars derives the cookie domain, CORS origin, and the **WebAuthn/passkey relying
...
- **Renewal is on you.** Unlike Let's Encrypt (which NPM auto-renews), a custom cert
  ...
  it against the NPM API). Blackfriars's Expiry & Rotation dashboard tracks *Blackfriars's* CA
...
- set `X-Forwarded-For` / `X-Forwarded-Proto`, and — so Blackfriars sees the real client
...
After bootstrap the host trusts the Blackfriars CA and every user connects with their
