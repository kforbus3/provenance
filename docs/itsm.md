...
Provenance can open a **change/incident ticket** in your IT service-management system for each
...
- When a user files an access request (Approvals → Request access), if the integration is enabled
  and the requester didn't already supply a ticket reference, Provenance opens a ticket describing the
...
- **Two-way sync**: when the request is later **approved or denied**, Provenance records the decision back
...
The token is **sealed at rest** (like Provenance's other integration secrets) and is never returned by
...
- A requester can still enter their own ticket reference on the request; Provenance only auto-opens one
