...
Provenance brokers privileged access to **databases** the same way it brokers SSH, RDP, and Kubernetes:
operators never hold the database credential. Register a database target, and users run SQL from the
**Databases** page — Provenance reaches the database **through the jump host**, injects a **vaulted
...
Each SQL engine runs its native driver over the single SSH-tunneled connection Provenance opens through
...
`{ "listDatabases": 1 }` — and Provenance returns the result as formatted JSON. Commands run against the
target's **Database name**, which is also the authentication database (typically `admin` for a
root/admin credential). The Mongo driver opens several connections, so Provenance gives each its own
...
- **Jump-host reachability.** Provenance dials the database through the jump host using the caller's
...
- **Least privilege.** Scope the vaulted database credential to what the brokered users should be
  able to do; Provenance's `Database.Connect` gate and any [access policies](./access-policies.md) apply on
  top.
