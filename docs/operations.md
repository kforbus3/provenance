...
Day-to-day operator flows for Provenance. Assumes the stack is up via `make up`.
...
2. Click the **Enroll** (cable) icon on the host row and choose how to reach the host for the
   ...
   - **Already trusts the Provenance CA** — re-provision a previously-enrolled host with the session cert.
...
subscribes to a WebSocket and updates in real time. Enrolled hosts are probed with Provenance's
...
> On a **Dynamic** group, **manual add/remove is disabled** — Provenance refuses it so membership
...
> Provenance checks: if the configured URL resolves to a public address, the settings page says so
...
Provenance can authenticate users against an external identity provider as well as
...
- Rules that touch SSH, the firewall, account lockout, networking sysctls (`ip_forward`,
  `rp_filter`, `route_localnet`, `send_redirects`, `ip_local_port_range`), or Provenance's privilege path
  (`sudo_*` such as `noexec`/`requiretty`, and direct/root-login lockout) are flagged **⚠
  access-impacting** because their fixes can sever Provenance's own access to or automation of the host —
...
> **Rebooting a host Provenance runs on top of** (a hypervisor, or the Provenance VM itself) kills the run
> that asked for it — Provenance dies mid-flight and the run is later reconciled as `interrupted`,
...
> on Provenance's side to wait with.

**Hosts with a vaulted credential** (routers, switches, appliances that don't trust the Provenance
CA) are supported: the runner injects the host's **vaulted SSH key or password** for the final
hop — the same credential the terminal uses — while the jump-host hop still uses the Provenance
...
reliable path is the **RouterOS binary API** (port 8728), which Provenance tunnels through the jump host.
...
2. In Provenance, add the router as a host, attach an **open-policy password credential**
...
When an approver resolves a request, Provenance notifies the configured admin channel **and** emails
