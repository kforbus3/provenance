...
Day-to-day operator flows for Blackfriars. Assumes the stack is up via `make up`.
...
## Upgrading Blackfriars (in-UI)

Blackfriars upgrades itself from a single signed **`.fleetup`** bundle — no SSH, no
...
2. Click the **Enroll** (cable) icon on the host row and choose how to reach the host for the
   ...
   - **Already trusts the Blackfriars CA** — re-provision a previously-enrolled host with the session cert.
...
subscribes to a WebSocket and updates in real time. Enrolled hosts are probed with Blackfriars's
...
> On a **Dynamic** group, **manual add/remove is disabled** — Blackfriars refuses it so membership
...
> Blackfriars checks: if the configured URL resolves to a public address, the settings page says so
...
Blackfriars can authenticate users against an external identity provider as well as
...
**normal sign-in form** — Blackfriars **falls back to LDAP when local auth fails**,
...
- Strict profiles (e.g. **ANSSI High**) run many filesystem-walking checks and can take **tens of
  ...
  Blackfriars caps a scan at the **scan timeout** — adjust it in **Settings → Security scans** (5–480 min;
...
- Rules that touch SSH, the firewall, account lockout, networking sysctls (`ip_forward`,
  `rp_filter`, `route_localnet`, `send_redirects`, `ip_local_port_range`), or Blackfriars's privilege path
  (`sudo_*` such as `noexec`/`requiretty`, and direct/root-login lockout) are flagged **⚠
  access-impacting** because their fixes can sever Blackfriars's own access to or automation of the host —
...
- Remediating a **control-plane host** — the jump host, a host tagged `control-plane`/`protected`, or
  ...
  box that runs Blackfriars (e.g. an `ip_forward=0` sysctl that breaks Docker's bridge networking) can lock
  Blackfriars out of the entire fleet; only proceed with out-of-band console access to recover. If it does
...
- The scan needs SCAP content matching the host's **OS version** (e.g. `ssg-debian13-ds.xml`
  for Debian 13). If a host's distro is newer than its packaged `scap-security-guide`, Blackfriars
...
sudo, so your plays should target **`hosts: all`** — Blackfriars supplies the inventory and limits it
...
> **Rebooting a host Blackfriars runs on top of** (a hypervisor, or the Blackfriars VM itself) kills the run
> that asked for it — Blackfriars dies mid-flight and the run is later reconciled as `interrupted`,
...
> on Blackfriars's side to wait with.

**Hosts with a vaulted credential** (routers, switches, appliances that don't trust the Blackfriars
CA) are supported: the runner injects the host's **vaulted SSH key or password** for the final
hop — the same credential the terminal uses — while the jump-host hop still uses the Blackfriars
...
reliable path is the **RouterOS binary API** (port 8728), which Blackfriars tunnels through the jump host.
...
2. In Blackfriars, add the router as a host, attach an **open-policy password credential**
...
only) upgrades Blackfriars in place from a single signed file. Choose a **`.fleetup`** bundle, review
...
**Check for updates (pull).** Instead of uploading, you can point Blackfriars at a signed release
...
When an approver resolves a request, Blackfriars notifies the configured admin channel **and** emails
