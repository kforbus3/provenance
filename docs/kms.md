...
Provenance seals its at-rest secrets — the SSH CA signing key (`ca_keys.private_enc`) and every
...
- The KMS key never leaves the KMS/HSM; Provenance only ever sends a small ciphertext to unwrap.
- Every unwrap is authorized and audited by the KMS itself (in addition to Provenance's own audit log).
...
On the next boot Provenance logs `master passphrases unsealed via external KMS` and operates normally.
If the KMS is unreachable or the key is wrong, boot **fails closed** — Provenance does not start with an
unusable CA/vault.
