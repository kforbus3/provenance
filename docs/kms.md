...
Blackfriars seals its at-rest secrets — the SSH CA signing key (`ca_keys.private_enc`) and every
...
**HSM** lets you keep them off disk in plaintext: you store only a KMS-**wrapped** blob, and Blackfriars
...
- The KMS key never leaves the KMS/HSM; Blackfriars only ever sends a small ciphertext to unwrap.
- Every unwrap is authorized and audited by the KMS itself (in addition to Blackfriars's own audit log).
...
On the next boot Blackfriars logs `master passphrases unsealed via external KMS` and operates normally.
If the KMS is unreachable or the key is wrong, boot **fails closed** — Blackfriars does not start with an
unusable CA/vault.
