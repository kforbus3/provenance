# Provenance — User Guide

Provenance lets you open SSH sessions and transfer files to your servers
...
1. Open the Provenance URL your administrator gave you.
...
- **OIDC:** if SSO is enabled, the login page shows a **Sign in with SSO** button.
  ...
  you're returned to Provenance already authenticated — no separate password
  to enter here.
...
- They're shown **only once**, right after you generate them. **Save them now** —
  print them or store them in your password manager. Provenance keeps only a
...
Provenance also brokers access to **databases** and **Kubernetes clusters** the same way — you never
handle the credential. On the **Databases** page, run SQL against a registered PostgreSQL, MySQL,
MariaDB, or SQL Server target (Provenance injects a vaulted credential and audits every query). On the
**Kubernetes** page, browse cluster resources or point `kubectl` at Provenance's proxy. These appear only
...
## 7. Ask Provenance — the AI assistant
...
*"and db-02?"* or *"what about last week?"* — and Provenance keeps the earlier context,
...
3. When you confirm, Provenance re-checks your permission and runs it, then shows the
...
Deny for each. On approval, Provenance re-checks that the original requester still has the
