# Blackfriars — User Guide

Blackfriars lets you open SSH sessions and transfer files to your servers
...
1. Open the Blackfriars URL your administrator gave you.
...
- **OIDC:** if SSO is enabled, the login page shows a **Sign in with SSO** button.
  ...
  you're returned to Blackfriars already authenticated — no separate password
  to enter here.
...
- They're shown **only once**, right after you generate them. **Save them now** —
  print them or store them in your password manager. Blackfriars keeps only a
...
Blackfriars also brokers access to **databases** and **Kubernetes clusters** the same way — you never
handle the credential. On the **Databases** page, run SQL against a registered PostgreSQL, MySQL,
MariaDB, or SQL Server target (Blackfriars injects a vaulted credential and audits every query). On the
**Kubernetes** page, browse cluster resources or point `kubectl` at Blackfriars's proxy. These appear only
...
## 7. Ask Blackfriars — the AI assistant
...
*"and db-02?"* or *"what about last week?"* — and Blackfriars keeps the earlier context,
...
3. When you confirm, Blackfriars re-checks your permission and runs it, then shows the
...
Deny for each. On approval, Blackfriars re-checks that the original requester still has the
